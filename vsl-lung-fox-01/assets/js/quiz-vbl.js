...
document.querySelectorAll(".option").forEach(option => {
  option.addEventListener("click", (event) => {
    ...
    // Adicione esta linha se quiser que avance automaticamente:
    const currentQuestion = parseInt(questionId.replace("question", ""));
    showNextQuestion(currentQuestion, currentQuestion + 1);
  });
});
